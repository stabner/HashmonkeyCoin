...
*Setup instructions for a Gitian build of HashmonkeyCoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the HashmonkeyCoin
...
- [Building HashmonkeyCoin Core](#building-hashmonkeycoin-core)
...
Clone the git repositories for HashmonkeyCoin Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/raptor3um/hashmonkeycoin
...
in the HashmonkeyCoin Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building HashmonkeyCoin Core
----------------

To build HashmonkeyCoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the HashmonkeyCoin Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/hashmonkeycoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/raptor3um/hashmonkeycoin
...
For example:
```bash
URL=https://github.com/crowning-/hashmonkeycoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit hashmonkeycoin=${COMMIT} --url hashmonkeycoin=${URL} ../hashmonkeycoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit hashmonkeycoin=${COMMIT} --url hashmonkeycoin=${URL} ../hashmonkeycoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit hashmonkeycoin=${COMMIT} --url hashmonkeycoin=${URL} ../hashmonkeycoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the hashmonkeycoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../hashmonkeycoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/hashmonkeycoin
SIGPATH=/some/root/path/hashmonkeycoin-detached-sigs

./bin/gbuild --url hashmonkeycoin=${BTCPATH},signature=${SIGPATH} ../hashmonkeycoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/hashmonkeycoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/hashmonkeycoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/hashmonkeycoin-osx-build.assert
```
...
[hashmonkeycoin/gitian.sigs](https://github.com/raptor3um/gitian.sigs/) repository, or if that's not possible to create a pull
request.
