Cross-compiliation of hashmonkeycoin Core
===============================

hashmonkeycoin Core can be cross-compiled on Linux to all other supported host systems. This is done by changing
the `HOST` parameter when building the dependencies and then specifying another `--prefix` directory when building hashmonkeycoin.
...
When building hashmonkeycoin Core, use
...
example /usr/src/hashmonkeycoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
It's not possible to build the hashmonkeycoin Core code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the hashmonkeycoin Core source code).
