...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/HashmonkeyCoinParams"
else
    PARAMS_DIR="$HOME/.hashmonkeycoin-params"
fi
...
DOWNLOAD_URL="https://github.com/hashmonkeycoin/hashmonkeycoin/releases/tag/1.0.0"
...
HMNY_DISABLE_WGET="${HMNY_DISABLE_WGET:-}"
#HMNY_DISABLE_IPFS="${HMNY_DISABLE_IPFS:-}"
HMNY_DISABLE_CURL="${HMNY_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$HMNY_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$HMNY_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$HMNY_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the HashmonkeyCoin parameters!
...
main() {
    ...
    cat <<EOF
HMNY - fetch-params.sh

This script will fetch the HashmonkeyCoin SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common HashmonkeyCoin SNARK parameters. Note that it is
