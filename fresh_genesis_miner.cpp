...
std::string generate_random_pubkey() {
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<> dis(0, 15);
    
    std::stringstream ss;
    ss << "04"; // Compressed public key prefix
    for (int i = 0; i < 64; i++) {
        ss << std::hex << dis(gen);
    }
    return ss.str();
...
uint32_t generate_random_timestamp() {
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<uint32_t> dis(1759700000, 1759800000); // Random timestamp in 2025
    return dis(gen);
}

// Generate a random suffix for the timestamp message
...
int main() {
    std::cout << "HashmonkeyCoin FRESH Genesis Block Miner" << std::endl;
    std::cout << "=======================================" << std::endl;
    std::cout << "This tool will generate COMPLETELY FRESH genesis blocks with unique parameters." << std::endl;
    std::cout << "Each run will produce different timestamps, public keys, and nonces." << std::endl;

    // Generate COMPLETELY FRESH parameters for mainnet
    uint32_t mainnet_nTime = generate_random_timestamp();
    std::string mainnet_suffix = generate_random_suffix();
    std::string mainnet_pubkey = generate_random_pubkey();
    
    const char* mainnet_pszTimestamp = ("HashmonkeyCoin Genesis Block - " + std::to_string(mainnet_nTime) + " - Unique Blockchain Launch - " + mainnet_suffix).c_str();
    const std::string mainnet_genesisOutputScript = mainnet_pubkey;
    ...
    // Generate COMPLETELY FRESH parameters for testnet
    uint32_t testnet_nTime = generate_random_timestamp();
    std::string testnet_suffix = generate_random_suffix();
    std::string testnet_pubkey = generate_random_pubkey();
    
    const char* testnet_pszTimestamp = ("HashmonkeyCoin Genesis Block - " + std::to_string(testnet_nTime) + " - Unique Blockchain Launch - " + testnet_suffix).c_str();
    const std::string testnet_genesisOutputScript = testnet_pubkey;
