...
GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params &consensusParams, bool fSuperblockPartOnly) {
    // HashmonkeyCoin uses a simple Bitcoin-like halving system
    // Initial reward: 500 HMNY, halves every 210,000 blocks
    
    CAmount nSubsidy = 500 * COIN;  // Initial reward: 500 HMNY
    
    // Calculate number of halvings that have occurred
    int nHalvings = nPrevHeight / consensusParams.nSubsidyHalvingInterval;
    
    // Halve the subsidy for each halving period
    // Use right shift for safe division by powers of 2
    nSubsidy >>= nHalvings;
    
    // Ensure minimum reward of 1 satoshi (0.00000001 HMNY)
    if (nSubsidy < 1) {
        nSubsidy = 1;
    }
...
CAmount GetSmartnodePayment(int nHeight, CAmount blockValue, CAmount specialTxFees) {
    // HashmonkeyCoin: Smartnode payments disabled - full reward goes to PoW miners
    return 0;
}
