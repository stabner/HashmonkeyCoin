...
std::string LicenseInfo() {
    const std::string URL_SOURCE_CODE = "<https://github.com/stabner/HashmonkeyCoin>";
    const std::string URL_WEBSITE = "<https://hashmonkeys.cloud>";
...
bool AppInitMain(const util::Ref &context, NodeContext &node, interfaces::BlockAndHeaderTipInfo *tip_info) {
    ...
    for (const auto &address: vSporkAddresses) {
        if (!sporkManager.SetSporkAddress(address)) {
            // During initial setup, allow invalid addresses as a warning for all networks
            // This allows the daemon to start and generate proper addresses
            // Mainnet, testnet, and devnet can all start without valid spork keys initially
